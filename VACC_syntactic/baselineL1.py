...
class VAE(nn.Module):
  ...
  def __init__(self):
        ...
        self.bn2 = nn.BatchNorm1d(self.hidden_size1)
        self.fc3 = nn.Linear(self.hidden_size1, zdim)
        self.bn3 = nn.BatchNorm1d(zdim)


        self.fc4 = nn.Linear(zdim, self.hidden_size1)
        self.bn4 = nn.BatchNorm1d(self.hidden_size1)
        self.fc5 = nn.Linear(self.hidden_size1, self.hidden_size)
        self.bn5 = nn.BatchNorm1d(self.hidden_size)
        self.fc6 = nn.Linear(self.hidden_size, self.input_size)
        self.bn6 = nn.BatchNorm1d(self.input_size)
...
if __name__ == '__main__':
  ...
  dataset = EntDataset('/home/jay_kejriwal/Fisher/Processed/h5/VACC/syntactic/train_nonorm.h5')
  ...
  for fold, (train_ids, test_ids) in enumerate(kfold.split(dataset)):
    ...
    with torch.no_grad():
        ...
        for idx, (data,Y_test_r1,Y_test_r2,Y_test_r3,Y_test_r4,Y_test_r5,Y_test_r6,Y_test_r7,Y_test_r8,Y_test_r9,Y_test_r10) in enumerate(zip(X_Y_test,Y_test_rand1,Y_test_rand2,Y_test_rand3,Y_test_rand4,Y_test_rand5,Y_test_rand6,Y_test_rand7,Y_test_rand8,Y_test_rand9,Y_test_rand10)):
            ...
            loss_real = lp_distance(x_data, y_data).item()
            loss_fake1 = lp_distance(x_data, y_fake_data1).item()
            loss_fake2 = lp_distance(x_data, y_fake_data2).item()
            loss_fake3 = lp_distance(x_data, y_fake_data3).item()
            loss_fake4 = lp_distance(x_data, y_fake_data4).item()
            loss_fake5 = lp_distance(x_data, y_fake_data5).item()
            loss_fake6 = lp_distance(x_data, y_fake_data6).item()
            loss_fake7 = lp_distance(x_data, y_fake_data7).item()
            loss_fake8 = lp_distance(x_data, y_fake_data8).item()
            loss_fake9 = lp_distance(x_data, y_fake_data9).item()
            loss_fake10 = lp_distance(x_data, y_fake_data10).item()
